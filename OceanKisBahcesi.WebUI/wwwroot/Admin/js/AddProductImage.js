...
function updateProduct2DImage() {
    var data = new FormData();

    data.append('file', $('input[type=file]')[0].files[0]);
    data.append("Path", $("#path").val());
    jQuery.ajax({
        url: '/Admin/AddSubProduct2DImage',
        data: data,
        cache: false,
        contentType: false,
        processData: false,
...
function addProduct2DImage() {
    var data = new FormData();

    data.append('file', $('input[type=file]')[0].files[0]);
    data.append("Path", $("#path").val());
    jQuery.ajax({
        url: '/Admin/AddProduct2DImage',
        data: data,
        cache: false,
        contentType: false,
        processData: false,
...
function deleteSubProduct2DImage(id) {
    ...
    Swal.fire({
        ...
        /* Read more about isConfirmed, isDenied below */
        if (result.isConfirmed) {
            $.post("/Admin/DeleteSubProduct2DImage", {
                "id": id
            }, function (res) {
                if (res.success == true) {
                    window.location.reload();
